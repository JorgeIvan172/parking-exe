...
class ParkingApp:
    ...
    def create_ticket_ui(self):
        ...
        qr_path = generate_ticket_qr(ticket_id)
    ...
    def close_ticket_ui(self):
        ...
        if not plate:
            self.result_label.config(text="⚠️ Ingresa una placa.")
            return
    ...
    def show_tickets(self, parent_frame):
        ...
        conn = sqlite3.connect("parking.db")
        cursor = conn.cursor()
        cursor.execute("SELECT id, plate, entry_time, exit_time, paid FROM tickets ORDER BY id DESC")
